...
internal static class ConsoleAppEmitter
{
    ...
    {
        List<CommandMethodInfo> validCommands = [.. commands.OfType<CommandMethodInfo>()];
    ...
    )
    {
        ...
        foreach (CommandMethodInfo? cmd in commands)
        {
            if (cmd is null)
                continue;
    ...
    )
    {
        ...
        List<IGrouping<string, CommandMethodInfo>> containingClasses = [.. validCommands.GroupBy(c => c.ContainingClassFullName)];

        if (containingClasses.Count > 0)
        {
            for (int i = 0; i < containingClasses.Count; i++)
            {
                IGrouping<string, CommandMethodInfo> group = containingClasses[i];
                string prefix = i == 0 ? "if" : "else if";
                CommandMethodInfo firstCmd = group.First();
                ...
                foreach (CommandMethodInfo cmd in group)
                {
                    string factoryName = $"Create{firstCmd.ContainingClassName}_{cmd.MethodName}";
                    if (cmd.IsRoot)
                    {
    ...
    {
        string returnType = cmd.IsRoot ? "RootCommand" : "Command";
        string factoryName = $"Create{cmd.ContainingClassName}_{cmd.MethodName}";
        ...
        foreach (string optTypeName in cmd.OptionsTypeNames)
        {
            if (optionsTypes.TryGetValue(optTypeName, out OptionsTypeInfo? optType))
            {
                string mapperName = Utilities.GetMapperName(optType);
        ...
        foreach (string optTypeName in cmd.OptionsTypeNames)
        {
            ...
            {
                string mapperName = Utilities.GetMapperName(optType);
                string optVarName = $"options{optIdx}";
        ...
        // Call the method
        string methodArgs = string.Join(", ", optionVarNames);
        if (cmd.IsAsync)
        {
